...
public class TestCorrectiveInvoice
    extends AbstractAccountingPrintTest<CorrectiveInvoice, CorrectiveInvoicePosition> {

  public static Function<Integer, Consumer<CorrectiveInvoicePosition>> createCorrectivePosition =
      i ->
          correctiveInvoicePosition(
              "spaceholder 2 " + i, bd(100).add(bd(i)), bd("0.1").add(bd(i)), "14");

  @Test
  public void full() throws Exception {
    testCreatePDF(
        getFullCorrectiveInvoice(),
        (html, pdf) -> {
    ...
    return createAccountingPrint(
        compose(
            ...
            invoice -> {
              ...
              invoice.setAmountDueAfterCorrectionInWords(
                  speakCashAmount(sumAfterCorrection.getSum()));
  ...
  Consumer<CorrectiveInvoicePosition>[] positions() {
    return flatten(
            asList(
                invoicePosition(
                    "lek. med. Leszek Kowalski - RTG Klatki piersiowej", bd("2"), bd("3.21"), "22"),
